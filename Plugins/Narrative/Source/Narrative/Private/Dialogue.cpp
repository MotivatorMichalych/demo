...
void UDialogue::UpdateSpeakerRotations()
{
	for (auto& SpeakerActorKVP : SpeakerAvatars)
	{
		if (AActor* Avatar = SpeakerActorKVP.Value)
		{
			...
			else if(CurrentListenerAvatar)//If we are the speaker, face the listener 
			{
...
bool UDialogue::IsShared() const
{
	return OwningComp && OwningComp->IsSharedComponent();
}

void UDialogue::SetSharedDialoguePlayerAvatar(class APawn* PawnOverride)
{
	SharedDialoguePlayerAvatar = PawnOverride;
}
...
AActor* UDialogue::GetPlayerAvatar() const
{
	if (SharedDialoguePlayerAvatar)
	{
		return SharedDialoguePlayerAvatar;
	}
...
FVector UDialogue::GetSpeakerHeadLocation_Implementation(class AActor* Actor) const
{
	...
	{	
		if (USkeletalMeshComponent* SkelMesh = Cast<USkeletalMeshComponent>(ActorSkelMesh))
		{
			...
			if (!SkelMesh->ComponentHasTag(NAME_FaceTag))
			{
...
void UDialogue::PlayDialogueShot(class UNarrativeDialogueShot* Shot, class AActor* Speaker, class AActor* Listener)
{
	...
	UE_LOG(LogNarrative, Log, TEXT("Dialogue %s tried playing a dialogue shot. These are deprecated - you need to select a dialogue sequence instead."), *GetNameSafe(this));
