...
bool UNarrativeComponent::IsSharedComponent() const
{
	return GroupMembers.Num() > 0;
}
...
bool UNarrativeComponent::HasDialogueAvailable(TSubclassOf<class UDialogue> DialogueClass, class AActor* DefaultAvatar, FName StartFromID /*= NAME_None*/)
{
	if (IsValid(DialogueClass))
	{
		return MakeDialogue(DialogueClass, DefaultAvatar, StartFromID) != nullptr;
	}
	...
	if (HasAuthority())
	{
		...
		{	
			...
			if (CurrentDialogue)
			{
				...
				//If we're a shared comp, tell all the group members to begin the quest
				if (IsSharedComponent())
				{
					for (auto& GroupMember : GroupMembers)
					{
						if (GroupMember)
						{
							GroupMember->ClientBeginSharedDialogue(DialogueClass, DefaultAvatar, CurrentDialogue->MakeIDsFromNPCNodes(CurrentDialogue->NPCReplyChain), CurrentDialogue->MakeIDsFromPlayerNodes(CurrentDialogue->AvailableResponses));
						}
					}
				}
				else
				{
					//If this is a shared dialogue, tell all the shared players to begin
...
void UNarrativeComponent::ClientBeginSharedDialogue_Implementation(TSubclassOf<class UDialogue> Dialogue, class AActor* NPC, const TArray<FName>& NPCReplyChainIDs, const TArray<FName>& AvailableResponseIDs)
{
	if (SharedNarrativeComp)
	{
		if (IsValid(Dialogue))
		{
			UDialogue* SharedDialogue = SharedNarrativeComp->CurrentDialogue;
			...
			if (!HasAuthority())
			{
				...
				if (SharedDialogue)
				{
					SharedNarrativeComp->OnDialogueFinished.Broadcast(SharedDialogue);

					SharedDialogue->Deinitialize();
					SharedNarrativeComp->CurrentDialogue = nullptr;
				}

				//Attempt to begin the dialogue, and if successful inform client to do the same 
				SharedDialogue = SharedNarrativeComp->CurrentDialogue = SharedNarrativeComp->MakeDialogue(Dialogue, NPC);

				if (SharedDialogue)
				{
					SharedDialogue->OwningController = GetOwningController();
					SharedDialogue->OwningPawn = GetOwningPawn();

					//Created dialogue won't have a valid chunk yet on the client - use the servers authed chunk it sent
					SharedDialogue->ClientReceiveDialogueChunk(NPCReplyChainIDs, AvailableResponseIDs);
					SharedNarrativeComp->OnDialogueBegan.Broadcast(SharedDialogue);
				}
			}
			else //Authority actually already did all this in BeginDialogue, however its OwningController/Pawn still needs set
			{
				if (SharedDialogue)
				{
					SharedDialogue->OwningController = GetOwningController();
...
void UNarrativeComponent::ClientRecieveDialogueChunk_Implementation(const TArray<FName>& NPCReplyChainIDs, const TArray<FName>& AvailableResponseIDs)
{
	if (!HasAuthority())
	{
		UDialogue* DialogueToUpdate = GetCurrentDialogue();

		if (DialogueToUpdate)
		{
			DialogueToUpdate->ClientReceiveDialogueChunk(NPCReplyChainIDs, AvailableResponseIDs);
		}
	}
}
...
{
	UDialogue* DialogueToExit = GetCurrentDialogue();

	if (DialogueToExit && DialogueToExit->bCanBeExited)
	{
		if(HasAuthority())
		{
			ExitDialogue();
...
void UNarrativeComponent::ClientSelectDialogueOption_Implementation(const FName& OptionID, class APawn* Selector = nullptr)
{
	if (CurrentDialogue)
	{
		if (UDialogueNode_Player* Option = CurrentDialogue->GetPlayerReplyByID(OptionID))
		{
			...
			if (Selector)
			{
				CurrentDialogue->SetSharedDialoguePlayerAvatar(Selector);
			}
	...
	{
		if (UDialogueNode_Player* Option = SharedNarrativeComp->CurrentDialogue->GetPlayerReplyByID(OptionID))
		{
			//We need to aim the camera at the person that actually said the line 
			if (Selector)
			{
				SharedNarrativeComp->CurrentDialogue->SetSharedDialoguePlayerAvatar(Selector);
			}
...
void UNarrativeComponent::ServerSelectDialogueOption_Implementation(const FName& OptionID)
{
	...
	{
		//Someone in a group dialogue selected an option, inform everyone in the group to select the dialogue option but view their character
		if (CurrentDialogue && CurrentDialogue->OwningComp && CurrentDialogue->OwningComp->IsSharedComponent())
		{
			for (auto& GroupMember : CurrentDialogue->OwningComp->GroupMembers)
			{
				//selecting member will have already started the dialogue option 
				if (GroupMember && GroupMember != this)
				{
					//GroupMember->ClientSelectDialogueOption(OptionID, GetOwningPawn());
				}
			}
		}
...
void UNarrativeComponent::ServerSelectSharedDialogueOption_Implementation(const FName& OptionID)
{
	if (SharedNarrativeComp && SharedNarrativeComp->CurrentDialogue)
	{
		if (UDialogueNode_Player* OptionNode = SharedNarrativeComp->CurrentDialogue->GetPlayerReplyByID(OptionID))
		{
			SharedNarrativeComp->CurrentDialogue->SelectDialogueOption(OptionNode);
		}
						...
						if (Update.IntPayload.IsValidIndex(0) && Update.IntPayload.IsValidIndex(1))
						{
							...
							if (UQuest* Quest = GetQuest(Update.QuestClass))
							{
								...
								if (UQuestBranch* Branch = Quest->GetBranch(BranchID))
								{
									...
									{
										Branch->QuestTasks[TaskIndex]->SetProgress(NewProgress);
									}
								}
							}
						}
...
class UDialogue* UNarrativeComponent::GetCurrentDialogue() const
{
	...
	{
		return SharedNarrativeComp->CurrentDialogue;
	}
...
void UNarrativeComponent::SetSharedNarrativeComponent(UNarrativeComponent* NewSharedNarrativeComponent)
{
	if (SharedNarrativeComp)
	{
		SharedNarrativeComp->GroupMembers.Remove(this);
	}
	...
	if (SharedNarrativeComp)
	{
		SharedNarrativeComp->GroupMembers.AddUnique(this);
	}
}
